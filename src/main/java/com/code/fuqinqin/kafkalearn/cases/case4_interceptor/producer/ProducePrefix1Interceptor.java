 ...
 */
@Slf4j
public class ProducePrefix1Interceptor implements ProducerInterceptor<String, String> {
    /**
     * 消息发送成功的统计项
     */
    ...
    public void onAcknowledgement(RecordMetadata recordMetadata, Exception exception) {
        ...
        log.info("[拦截器1]发送成功={}条，发送失败={}条，发送成功率={}",
    ...
    public void close() {
        double successRatio = (double) successCounter.get() / (successCounter.get() + failCounter.get());
        log.info("[拦截器1-close]发送成功率={}", successRatio * 100 + "%");
    }
